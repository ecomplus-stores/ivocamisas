...
export default {
  ...
  computed: {
    ...
    videoSrc () {
      const video = this.video || (this.product.videos && this.product.videos[0] || { url: ''})
      //console.log(`video`,video)
      
      //video.url = 'https://ivocamisas.b-cdn.net/Ajax%20Home%202024-25.mp4'
      // if(video.url.includes(`ivocamisas.b-cdn.net/`)){
      //     return video.url
      // }
      if (video && video.url) {
  ...
  methods: {
    getImg,
    toggleVideoPopup(status,origin){
      console.log(status, window.innerWidth, origin)
      this.videoPopup = (origin == "video" && window.innerWidth < 990) ? true : status
    },
    isMobile(){
